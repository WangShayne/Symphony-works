defmodule SymphonyElixir.SourceControl do
  ...
  alias SymphonyElixir.SourceControl.{EffectAdapter, Fake, GitHub, GitLab, Transport}
  ...
  @doc """
  Journals and executes one durable Source Control mutation.

  The effect provider and target are derived from the persisted mutation intent;
  caller-supplied values cannot relabel the external operation. Runtime
  credentials and transports are removed before the Effect Record is written.
  """
  @spec execute_effect(map()) ::
          {:ok, SymphonyElixir.Effects.Record.t()} | {:error, atom()}
  def execute_effect(attrs), do: execute_effect(attrs, [])

  @spec execute_effect(map(), keyword()) ::
          {:ok, SymphonyElixir.Effects.Record.t()} | {:error, atom()}
  def execute_effect(attrs, opts) when is_map(attrs) and is_list(opts) do
    with {:ok, effect_attrs} <- EffectAdapter.prepare(attrs) do
      Effects.execute(effect_attrs, EffectAdapter, opts)
    end
  end

  def execute_effect(_attrs, _opts), do: {:error, :invalid_effect}
