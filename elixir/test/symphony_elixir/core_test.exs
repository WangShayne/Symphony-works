defmodule SymphonyElixir.CoreTest do
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    Process.sleep(50)
    state = :sys.get_state(pid)
    retry_observed_at_ms = System.monotonic_time(:millisecond)

    refute Map.has_key?(state.running, issue_id)
    assert MapSet.member?(state.completed, issue_id)
    assert %{attempt: 1, due_at_ms: due_at_ms} = state.retry_attempts[issue_id]
    assert is_integer(due_at_ms)
    assert_due_after_delay(due_at_ms, retry_requested_at_ms, retry_observed_at_ms, 1_000)
  end
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    assert_due_after_delay(due_at_ms, retry_requested_at_ms, retry_observed_at_ms, 40_000)
  end
  ...
  test "first abnormal worker exit waits before retrying" do
    ...
    assert_due_after_delay(due_at_ms, retry_requested_at_ms, retry_observed_at_ms, 10_000)
  end
